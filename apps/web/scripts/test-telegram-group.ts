...
if (!newGroupId) {
  console.error("usage: <script> <chat_id>");
  process.exit(1);
}
...
async function main() {
  ...
  // 1. getChat to verify bot can see the chat
  ...
  if (!info.ok) {
    console.error("Bot cannot see this chat. Add the bot to the group first.");
    process.exit(2);
  }
  ...
  const send = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
    ...
    body: JSON.stringify({ chat_id: newGroupId, text: "✅ Test: poster เชื่อมต่อห้องนี้สำเร็จ" }),
  }).then(r => r.json());
